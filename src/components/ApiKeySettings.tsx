...
import { faKey, faCheck, faExclamationCircle, faEye, faEyeSlash, faInfoCircle, faRobot, faSliders, faExchangeAlt, faChevronDown, faGripLines } from '@fortawesome/free-solid-svg-icons';
...
const ApiKeySettings = ({ onClose }: ApiKeySettingsProps) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-red-700">
                <p>{apiKeyError}</p>
                <p className="mt-2">請確認相應的環境變數已正確設定。</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Display appropriate API key based on model provider */}
          ...
          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-blue-700">模型：</span>
                <span className="font-medium text-blue-800">{selectedModel.name}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-blue-700">每 1,000 tokens 成本：</span>
                <span className="font-medium text-blue-800">${selectedModel.costPer1kTokens} {selectedModel.currency}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-blue-700">預設最大 tokens：</span>
                <span className="font-medium text-blue-800">{selectedModel.defaultTokens}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-blue-700">滿載成本估算：</span>
                <span className="font-medium text-blue-800">
                  ${((selectedModel.costPer1kTokens * selectedModel.defaultTokens) / 1000).toFixed(4)} {selectedModel.currency}
                </span>
              </div>
            </div>
            <p className="mt-3 text-xs text-blue-600">
              實際成本將根據生成的文本長度而有所不同。上述預估僅供參考。
            </p>
          </div>
        )}
      </div>
